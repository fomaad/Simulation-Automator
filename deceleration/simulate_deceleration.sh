...
PROJECT="$PROJECT_ROOT/deceleration"

# paths
PYTHON_SCRIPT= "$PROJECT/scripts/simulate_next_combination_deceleration.py"
TEMP_SCRIPT= "$PROJECT/temp/decelerationTemp.script"
...
while true; do
    ...
    DEO_LINE=$(grep -oP 'deo = \K[0-9.]+' "$TEMP_SCRIPT")

    if [[ -z "$DX_LINE" || -z "$VE_LINE" || -z "$DEO_LINE" ]]; then
    ...
    DEO_TAG=$(awk "BEGIN { printf \"%d\", $DEO_LINE * 1000 }")

    OUTNAME="deceleration${DIST}-${VE_KMH}-${DEO_TAG}"
